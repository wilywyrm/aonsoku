export function byteSlice(value: string, byteStart: number, byteEnd: number): string {
  if (!value) return ''
  if (byteStart > byteEnd) {
    if (import.meta.env.DEV) console.warn('[byteSlice] byteStart > byteEnd', { byteStart, byteEnd })
    return ''
  }
  ...
  return new TextDecoder('utf-8', { fatal: false }).decode(encoded.slice(start, end + 1))
}
